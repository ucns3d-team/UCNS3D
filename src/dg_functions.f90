...
BASIS_TEMP = BASIS_REC2D(N, X1, Y1, NUMBER, ICONSIDERED, NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)
else
BASIS_TEMP = BASIS_REC(N, X1, Y1,z1, NUMBER, ICONSIDERED, NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)
end if
...
!> This function returns the derivative of the DG solution at a given point (X1, Y1, [Z1])
    ...
    DO I_VAR = 1, NOF_VARIABLES
        DO I_DIM = 1, DIMENSIONA
            IF (BR2_YN == 1) THEN
                IF(DIMENSIONA.EQ.2)THEN
                    DG_SOL_DER(I_VAR, I_DIM) = U_C(ICONSIDERED)%BR2_AUX_VAR(1,I_VAR,I_DIM) + DOT_PRODUCT(BASIS_REC2D(N, X1, Y1, NUMBER, ICONSIDERED, NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg), U_C(ICONSIDERED)%BR2_AUX_VAR(2:NUM_DG_DOFS,I_VAR,I_DIM))
                ELSE
                    DG_SOL_DER(I_VAR, I_DIM) = U_C(ICONSIDERED)%BR2_AUX_VAR(1,I_VAR,I_DIM) + DOT_PRODUCT(BASIS_REC(N, X1, Y1, Z1, NUMBER, ICONSIDERED, NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg), U_C(ICONSIDERED)%BR2_AUX_VAR(2:NUM_DG_DOFS,I_VAR,I_DIM))
                 END IF
            ELSE
...
DO I = 1, NOF_VARIABLES
    ...
    DG_SURF_FLUX(2:NUMBER_OF_DOG+1,I) = rHLLCFLUX(I) * WEIGHTS_TEMP(POINTX)*IELEM(N,ICONSIDERED)%SURF(FACEX)*BASIS_REC2D(N,X1,Y1,NUMBER,ICONSIDERED,NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)
...
DO I = 1, NOF_VARIABLES
    ...
    DG_SURF_FLUX(2:NUMBER_OF_DOG+1,I) = rHLLCFLUX(I) * WEIGHTS_TEMP(POINTX)*IELEM(N,ICONSIDERED)%SURF(FACEX)*BASIS_REC(N,X1,Y1,z1,NUMBER,ICONSIDERED,NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)
END DO
...
DO I = 1, NOF_VARIABLES
    ...
    DG_SURF_FLUXV(2:NUMBER_OF_DOG+1,I) = HLLCFLUX(I) * WEIGHTS_TEMP(POINTX)*IELEM(N,ICONSIDERED)%SURF(FACEX)*BASIS_REC2D(N,X1,Y1,NUMBER,ICONSIDERED,NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)
...
DO I = 1, NOF_VARIABLES
    ...
    DG_SURF_FLUXV(2:NUMBER_OF_DOG+1,I) = HLLCFLUX(I) * WEIGHTS_TEMP(POINTX)*IELEM(N,ICONSIDERED)%SURF(FACEX)*BASIS_REC(N,X1,Y1,z1,NUMBER,ICONSIDERED,NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)
END DO
...
!                    
                    ...
                    if (dimensiona.eq.2)then
                    NUMBER=IELEM(N,ICONSIDERED)%IORDER
                    tempint(1:IDEGFREE)=tempint(1:IDEGFREE)+(BASIS_REC2D(N,X1,Y1,number,IXX,IDEGFREE,icompwrt, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)*QP_ARRAY(I)%QP_WEIGHT(I_QP))
                    
                    else
                    NUMBER=IELEM(N,ICONSIDERED)%IORDER
                    tempint(1:IDEGFREE)=tempint(1:IDEGFREE)+(BASIS_REC(N,X1,Y1,z1,number,IXX,IDEGFREE,icompwrt, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)*QP_ARRAY(I)%QP_WEIGHT(I_QP))
        ...
        DO J_DOF = 1, NUM_DG_DOFS
            INTEG_MM = ZERO
             ...
             kron=1.0d0
                    ...
                    if (dimensiona.eq.2)then
                    NUMBER=IELEM(N,ICONSIDERED)%IORDER
                    BASIS_VECTOR(1:idegfree) = BASIS_REC2D(N,X1,Y1,number,IXX,NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)
                    else
                    NUMBER=IELEM(N,ICONSIDERED)%IORDER
                    BASIS_VECTOR(1:idegfree) = BASIS_REC(N,X1,Y1,z1,number,IXX,NUMBER_OF_DOG,ICOMPWRT, IELEM, ILOCAL_RECON3, INTEG_BASIS,integ_basis_dg)
