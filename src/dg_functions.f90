...
!> This function returns the derivative of the DG solution at a given point (X1, Y1, [Z1])
    ...
    DO I_VAR = 1, NOF_VARIABLES
        DO I_DIM = 1, DIMENSIONA
            IF (BR2_YN == 1) THEN
                IF(DIMENSIONA.EQ.2)THEN
                    DG_SOL_DER(I_VAR, I_DIM) = U_C(ICONSIDERED)%BR2_AUX_VAR(1,I_VAR,I_DIM) + DDOT_D(NUMBER_OF_DOG, BASIS_REC2D(N, X1, Y1, NUMBER, ICONSIDERED, NUMBER_OF_DOG),1, U_C(ICONSIDERED)%BR2_AUX_VAR(2:NUM_DG_DOFS,I_VAR,I_DIM),1)
                ELSE
                    DG_SOL_DER(I_VAR, I_DIM) = U_C(ICONSIDERED)%BR2_AUX_VAR(1,I_VAR,I_DIM) + DDOT_D(NUMBER_OF_DOG, BASIS_REC(N, X1, Y1, Z1, NUMBER, ICONSIDERED, NUMBER_OF_DOG),1, U_C(ICONSIDERED)%BR2_AUX_VAR(2:NUM_DG_DOFS,I_VAR,I_DIM),1)
                 END IF
            ELSE
                DG_SOL_DER(I_VAR, I_DIM) = DDOT_D(NUMBER_OF_DOG, BASIS_TEMP(1:NUMBER_OF_DOG,I_DIM),1, U_C(ICONSIDERED)%VALDG(1,I_VAR,2:NUM_DG_DOFS),1)
            END IF
        END DO
    END DO
