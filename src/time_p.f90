...
KMAXE=XMPIELRANK(N)
	...
	!!!$OMP DO REDUCTION (MIN:DT)
		...
		ELSE
		AGRT=SQRT(LEFTV(5)*gamma_g/LEFTV(1))
		END IF
        IF (RFRAME.eq.0) THEN
	...
	!!!$OMP DO REDUCTION (MIN:DT)
		...
		DT=MIN(DT,(CCFL/(2*IORDER+1))*(IELEM(N,I)%MINEDGE/((ABS(VELN))+(2.0D0*MAX(((4.0/3.0)*VISCL(1)/LEFTV(1)),gamma_g*LAML(1)/(PRANDTL*LEFTV(1)))*((2*IORDER+1)/IELEM(N,I)%MINEDGE)))))
...
KMAXE=XMPIELRANK(N)
	...
	!$OMP DO
		...
		IELEM(N,I)%DTL=(CCFL/(2*IORDER+1))*(IELEM(N,I)%MINEDGE/((ABS(VELN))+(2.0D0*MAX(((4.0/3.0)*VISCL(1)/LEFTV(1)),gamma_g*LAML(1)/(PRANDTL*LEFTV(1)))*((2*IORDER+1)/IELEM(N,I)%MINEDGE))))
...
KMAXE=XMPIELRANK(N)
	...
	!!!$OMP DO REDUCTION (MIN:DT)
		...
		ELSE
		AGRT=SQRT(LEFTV(4)*gamma_g/LEFTV(1))
		END IF
