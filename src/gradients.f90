...
IMPLICIT NONE
 ...
 SUBROUTINE ALLGRADS_INNER_SPHERE_CASE_ONLY(N, ICONSIDERED, imax_L, NUMBER_OF_DOG_L, NUMBER_OF_NEI_L, ANGLE1_L, ANGLE2_L, LEFTV_L, IELEM_L, U_C_L, ILOCAL_RECON3_L, ILOCAL_RECON5_L, IEXSOLHIR_L)
	!> @brief
	!> This subroutine calls the gradient approximation subroutines for every interior cell
	IMPLICIT NONE
	!$omp declare target
	INTEGER, INTENT(IN)::N, ICONSIDERED
	INTEGER, INTENT(INOUT)::imax_L, NUMBER_OF_DOG_L, NUMBER_OF_NEI_L
	REAL, INTENT(INOUT):: ANGLE1_L, ANGLE2_L
	REAL,DIMENSION(:), INTENT(INOUT) ::LEFTV_L
	TYPE(ELEMENT_NUMBER),DIMENSION(N:N,XMPIELRANK(N)), INTENT(INOUT)::IELEM_L
	TYPE(U_CENTRE),DIMENSION(:),INTENT(INOUT)::U_C_L
	TYPE(LOCAL_RECON3),DIMENSION(:),INTENT(INOUT)::ILOCAL_RECON3_L, ILOCAL_RECON5_L
	
	TYPE(EXCHANGE_SOLHI),DIMENSION(:), INTENT(INOUT)::IEXSOLHIR_L
	INTEGER::I
	I = iconsidered
	NUMBER_OF_DOG_L = IELEM_L(N, I)%IDEGFREE
	NUMBER_OF_NEI_L = IELEM_L(N, I)%inumneighbours
	imax_L = NUMBER_OF_NEI_L - 1
	...
	CASE (0)        !LEAST SQUARES EVERYTHING
		...
		CALL COMPUTE_GRADIENTS_MEAN_LSQ_SPHERE_CASE_ONLY(N, ICONSIDERED, NUMBER_OF_DOG_L, NUMBER_OF_NEI_L, imax_L, IELEM_L, U_C_L, ILOCAL_RECON3_L, ILOCAL_RECON5_L, IEXSOLHIR_L)
		CALL COMPUTE_GRADIENTS_INNER_MEAN_LSQ_VISCOUS_SPHERE_CASE_ONLY(N, ICONSIDERED, NUMBER_OF_DOG_L, NUMBER_OF_NEI_L, imax_L, LEFTV_L, IELEM_L, U_C_L, ILOCAL_RECON3_L, ILOCAL_RECON5_L, IEXSOLHIR_L)
																	  
		CALL COMPUTE_GRADIENTS_INNER_MEAN_GGS_VISCOUS_SPHERE_CASE_ONLY(N, ICONSIDERED, NUMBER_OF_DOG_L, NUMBER_OF_NEI_L, ANGLE1_L, ANGLE2_L, LEFTV_L, IELEM_L, U_C_L, ILOCAL_RECON3_L)
...
SUBROUTINE COMPUTE_GRADIENTS_INNER_MEAN_GGS_VISCOUS_SPHERE_CASE_ONLY(N, ICONSIDERED, NUMBER_OF_DOG_L, NUMBER_OF_NEI_L, ANGLE1_L, ANGLE2_L, LEFTV_L, IELEM_L, U_C_L, ILOCAL_RECON3_L)!check_all
	!> @brief
	!> This subroutine computes the gradients of the primitive variables of each interior cell using the Green-Gauss algorithm
	   IMPLICIT NONE
	   INTEGER, INTENT(IN)::N, ICONSIDERED, NUMBER_OF_DOG_L, NUMBER_OF_NEI_L
	   REAL, INTENT(INOUT):: ANGLE1_L, ANGLE2_L
	   REAL,DIMENSION(:), INTENT(INOUT) ::LEFTV_L
	   TYPE(ELEMENT_NUMBER),DIMENSION(N:N,XMPIELRANK(N)), INTENT(INOUT)::IELEM_L
	   TYPE(U_CENTRE),DIMENSION(:), INTENT(INOUT) ::U_C_L
	   ...
	   leftv_L(1:nof_variables) = U_C_L(I)%VAL(1, 1:nof_variables)
	   ...
	   DO J = 1, IELEM_L(N, I)%IFCA
		  ANGLE1_L = IELEM_L(N, I)%FACEANGLEX(J)
		  ANGLE2_L = IELEM_L(N, I)%FACEANGLEY(J)
		  NORMAL_ALL(1) = (COS(ANGLE1_L)*SIN(ANGLE2_L))
		  NORMAL_ALL(2) = (SIN(ANGLE1_L)*SIN(ANGLE2_L))
		  NORMAL_ALL(3) = (COS(ANGLE2_L))
	
		  leftv_L(1:nof_variables) = U_C_L(IELEM_L(N, I)%INEIGH(J))%VAL(1, 1:nof_variables)
		  call cons2prim_SPHERE_CASE_ONLY(n, LEFTV_L)
...
SUBROUTINE COMPUTE_GRADIENTS_MEAN_LSQ_SPHERE_CASE_ONLY(N, ICONSIDERED, NUMBER_OF_DOG, NUMBER_OF_NEI, imax_L, IELEM_L, U_C_L, ILOCAL_RECON3_L, ILOCAL_RECON5_L, IEXSOLHIR_L)!check all
	!> @brief
	!> This subroutine computes the gradients of the conserved variables of each cell using the least-squares
	   IMPLICIT NONE
	   INTEGER, INTENT(IN)::N, ICONSIDERED, NUMBER_OF_DOG, NUMBER_OF_NEI
	   INTEGER, INTENT(IN)::imax_L
	   TYPE(ELEMENT_NUMBER),DIMENSION(N:N,XMPIELRANK(N)), INTENT(INOUT)::IELEM_L
	   TYPE(U_CENTRE),DIMENSION(:), INTENT(INOUT)::U_C_L
	   TYPE(LOCAL_RECON3),DIMENSION(:), INTENT(INOUT)::ILOCAL_RECON3_L
	   ...
	   REAL, DIMENSION(nof_variables, IELEM_L(N, ICONSIDERED)%ADMIS)::SOLS2
	   REAL, DIMENSION(imax_L, nof_variables, IELEM_L(N, ICONSIDERED)%ADMIS)::MATRIX_1
	   ...
	   if (ILOCAL_RECON3_L(I)%LOCAL .eq. 1) then
		  ...
		  DO LL = 1, IELEM_L(N, I)%ADMIS; 
			 if (ll .eq. 1) then
				...
				CALL DGEMM_D(.FALSE., .FALSE., IELEM_L(N, I)%IDEGFREE, nof_variables, imax_L, &
						   ALPHA, ILOCAL_RECON3_L(I)%invmat_stencilt(1:IELEM_L(N, I)%IDEGFREE, 1:imax_L, LL), &
						   IELEM_L(N, I)%IDEGFREE, MATRIX_1(1:imax_L, 1:nof_variables, ll), &
						   imax_L, BETA, SOL_M(1:IELEM_L(N, I)%IDEGFREE, 1:nof_variables, ll), IELEM_L(N, I)%IDEGFREE)
	
			 ELSE
	
				CALL DGEMM_D(.FALSE., .FALSE., IDEGFREE2, nof_variables, numneighbours2 - 1, &
		  ...
		  DO LL = 1, IELEM_L(N, I)%ADMIS; 
			 if (ll .eq. 1) then
				ILOCAL_RECON5_L(I)%GRADIENTS(LL, 1:NUMBER_OF_DOG, 1:nof_variables) = SOL_M(1:NUMBER_OF_DOG, 1:nof_variables, ll)
	
			 ELSE
				ILOCAL_RECON5_L(I)%GRADIENTSC(LL, 1:IDEGFREE2, 1:nof_variables) = SOL_M(1:IDEGFREE2, 1:nof_variables, ll)
...
SUBROUTINE COMPUTE_GRADIENTS_INNER_MEAN_LSQ_VISCOUS_SPHERE_CASE_ONLY(N, ICONSIDERED, NUMBER_OF_DOG_L, NUMBER_OF_NEI_L, imax_L, LEFTV_L, IELEM_L, U_C_L, ILOCAL_RECON3_L, ILOCAL_RECON5_L, IEXSOLHIR_L)!check_all
	!> @brief
	!> This subroutine computes the gradients of the primitve variables of each interior cell using the least-squares
	   ...
	   REAL,DIMENSION(:), INTENT(INOUT) ::LEFTV_L
	   TYPE(ELEMENT_NUMBER),DIMENSION(N:N,XMPIELRANK(N)), INTENT(INOUT)::IELEM_L
	   TYPE(U_CENTRE),DIMENSION(:)::U_C_L
	   TYPE(LOCAL_RECON3),DIMENSION(:), INTENT(INOUT)::ILOCAL_RECON3_L
	   TYPE(LOCAL_RECON3),DIMENSION(:), INTENT(INOUT)::ILOCAL_RECON5_L
	   TYPE(EXCHANGE_SOLHI),DIMENSION(:), INTENT(INOUT)::IEXSOLHIR_L
	   REAL, DIMENSION(nof_variables)::SOLS1, SOLS2
	   REAL, DIMENSION(imax_L, nof_variables)::MATRIX_1
	   REAL, DIMENSION(NOF_VARIABLES, NUMBER_OF_DOG_L)::MATRIX_2
	   REAL, DIMENSION(NUMBER_OF_DOG_L, NOF_VARIABLES)::SOL_M
	   ...
	   if (ILOCAL_RECON3_L(I)%LOCAL .eq. 1) then
		  ...
		  MATRIX_1 = ZERO; MATRIX_2 = ZERO
		  LEFTV_L(1:nof_Variables) = U_C_L(ILOCAL_RECON3_L(I)%IHEXL(1, 1))%VAL(1, 1:nof_Variables)
		  CALL CONS2PRIM_SPHERE_CASE_ONLY(N, LEFTV_L)
		  ...
		  DO IQ = 1, imax_L
			 LEFTV_L(1:nof_Variables) = U_C_L(ILOCAL_RECON3_L(I)%IHEXL(1, IQ + 1))%VAL(1, 1:nof_Variables)
			 CALL CONS2PRIM_SPHERE_CASE_ONLY(N, LEFTV_L)
		  ...
		  CALL DGEMM_D(.FALSE., .FALSE., IELEM_L(N, I)%IDEGFREE, nof_variables, imax_L, &
					 ALPHA, ILOCAL_RECON3_L(I)%invmat_stencilt(1:IELEM_L(N, I)%IDEGFREE, 1:imax_L, LL), &
					 IELEM_L(N, I)%IDEGFREE, MATRIX_1(1:imax_L, 1:nof_variables), &
					 imax_L, BETA, SOL_M(1:IELEM_L(N, I)%IDEGFREE, 1:nof_variables), IELEM_L(N, I)%IDEGFREE)
	
		  DO VAR2 = 2, 4
			ILOCAL_RECON5_L(I)%VELOCITYDOF(VAR2 - 1, 1:NUMBER_OF_DOG_L) = SOL_M(1:NUMBER_OF_DOG_L, VAR2)
		  END DO
		  ILOCAL_RECON5_L(I)%GRADIENTSTEMP(1:NUMBER_OF_DOG_L) = SOL_M(1:NUMBER_OF_DOG_L, 1)
	   ...
	   ELSE
		  ...
		  DO IQ = 1, imax_L
		   ...
		   LEFTV_L(1:nof_Variables) = IEXSOLHIR_L(ILOCAL_RECON3_L(I)%IHEXN(1, IQ + 1))%SOL(ILOCAL_RECON3_L(I)%IHEXL(1, IQ + 1), 1:nof_Variables)
			 end if
			 CALL CONS2PRIM_SPHERE_CASE_ONLY(N, LEFTV_L)
			 SOLS2(2:4) = LEFTV_L(2:4)
			 SOLS2(1) = LEFTV_L(5)/LEFTV_L(1)
