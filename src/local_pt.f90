...
SUBROUTINE EXCHANGE_CORDX(N,INEEDT,TNEEDT,i_cnt4,i_cnt3)	!TT need to move to inoder4 if this is during timestepping
...
SUBROUTINE  LOCALISE_STEN2d(N,ICONSI,ILOX_IHEXG,ILOX_IHEXL,ILOX_IHEXB,ILOX_IHEXN,ILOX_ISHAPE,ILOX_XXC,ILOX_YYC,ILOX_ZZC,ILOX_VOLUME,ILOX_PERIODICFLAG,ILON_NODCOUNT,ILON_X,ILON_Y,ILON_Z)
	...
	REAL,DIMENSION(1:dimensiona,1:dimensiona)::VVA1
	REAL,DIMENSION(1)::DETA
	logical::this_loop, every_loop
	integer::left_index, right_index, helper
	...
	! !$OMP MASTER
		IF ((CODE_PROFILE.EQ.30).or.(MESH_MOVEMENT)) THEN
			DO NJ=1,IELEM(N,I)%nonodes
				...
				IF (ILOCAL_RECON3(I)%LOCAL.eq.1)then
					...
					DO L = 1, itarget
						...
						DO K=1,ielem(n,j)%nonodes    
							...
							DISTFD = min(DISTFD, SQRT(((X1X-X2X)**2)+((Y1Y-Y2Y - 1.0)**2)))
				...
				! do left_index = 1, (NOJCOUNT(NJ)-1)
				! 	do right_index = (left_index+1), NOJCOUNT(NJ)
				! 		if (IELEM(N,I)%NODES_NEIGHBOURS(NJ,left_index).gt.IELEM(N,I)%NODES_NEIGHBOURS(NJ,right_index)) then
				! 			helper = IELEM(N,I)%NODES_NEIGHBOURS(NJ,left_index)
				! 			IELEM(N,I)%NODES_NEIGHBOURS(NJ,left_index) = IELEM(N,I)%NODES_NEIGHBOURS(NJ,right_index)
				! 			IELEM(N,I)%NODES_NEIGHBOURS(NJ,right_index) = helper
				! 		end if
				! 	end do
				! end do
				! do left_index = 1, (NOJCOUNT(NJ)-1)
				! 	do while (IELEM(N,I)%NODES_NEIGHBOURS(NJ,left_index).eq.IELEM(N,I)%NODES_NEIGHBOURS(NJ,left_index+1))
				! 		print*,"trimming repeats"
				! 		do right_index = (left_index+1), (NOJCOUNT(NJ)-1)
				! 			IELEM(N,I)%NODES_NEIGHBOURS(NJ, right_index) = IELEM(N,I)%NODES_NEIGHBOURS(NJ, right_index+1)
				! 		end do
				! 		NOJCOUNT(NJ) = NOJCOUNT(NJ)-1
				! 	end do
				! end do
			...
			DO NJ=1,IELEM(N,I)%nonodes
				...
				!IF (IELEM(N,I)%NODES_NEIGHBOURS(NJ,J).GT.0)THEN
					...
					! if (IELEM(N,I)%NODES_NEIGHBOURS(NJ,J) == I) then
					! 	this_loop = .true.
					! end if
				!END IF
				END DO
				...
				! end if
			END DO
			! if (.not.every_loop) then
			! 	print*, "parent noode missing in the nodes neighnours in node", i, n
			! end if
