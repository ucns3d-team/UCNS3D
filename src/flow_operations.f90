...
FUNCTION BLEED2D(INITCOND,POX,POY)
!> @brief
!> This function applies a prescribed boundary condition to  the outflow in 3D
IMPLICIT NONE
...
CALL ROTATEF2d(N,TRI,CRIGHT_ROT,RIGHTV,ANGLE1,ANGLE2)
...
FUNCTION BLEED3D(INITCOND,POX,POY,POZ)
!> @brief
!> This function applies a prescribed boundary condition to  the outflow in 3D
IMPLICIT NONE
...
!
    ...
    CASE(99)    !BLEED BOUNDARY
     ...
     rightv(1:nof_Variables)=BLEED3D(INITCOND,pox,poy,poz)
...
! 							
    ...
    CASE(99)  !BLEED
     ...
     rightv(1:nof_Variables)=BLEED2d(INITCOND,pox,poy)
...
DO I=1,KMAXE
    ...
    END IF
    ELSE
    IF (U_C(I)%VAL(1,7).GT.0.05D0)THEN
        IF (IELEM(N,I)%XXC.LE.POST1)THEN
            POST1=IELEM(N,I)%XXC
            TRAJ1=I
        END IF
        IF (((IELEM(N,I)%YYC.LE.0.051).AND.(IELEM(N,I)%YYC.GE.0.049)))THEN
            IF(IELEM(N,I)%XXC.LE.POST2)THEN
                POST2=IELEM(N,I)%XXC
                TRAJ2=i
            end if
...
SUBROUTINE FLUX2DX
IMPLICIT NONE
REAL::P,U,V,W,E,R,S,GM,SKIN,IEN,PI, IE1, MP_STIFF, MP_DENSITY
IF(MULTISPECIES.EQ.1)THEN
