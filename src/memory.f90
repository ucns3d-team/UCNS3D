...
! ALLOCATE(SHT_RG(KMAXE,1:nof_Variables))
! END IF
...
END IF
ALLOCATE (ILOCAL_RECON5(i)%gradf(nof_variables-1,IDEGFREE))!30
...
DO II=1,NOF_INTERIOR	!for all the interior elements
	...
	END IF
	ALLOCATE (ILOCAL_RECON3(I)%GRADS(nof_Variables-1+TURBULENCEEQUATIONS+passivescalar+(QSAS_MODEL*3),3))
...
DO I=1,KMAXE
	...
	IF (ITESTCASE.EQ.4)THEN !Linear step?
        ...
        ALLOCATE (ILOCAL_RECON3(I)%GRADS(nof_Variables-1+TURBULENCEEQUATIONS+passivescalar+(QSAS_MODEL*2),2))
	END IF
