...
!> This subroutine allocates memory for vertices
	...
	ALLOCATE(SOLUTION_INTEG2(1:NOF_vARIABLES),SOLUTION_INTEG_STRONG(1:NOF_vARIABLES),SOLUTION_INTEG_WEAK(1:NOF_vARIABLES))
	...
	ALLOCATE(RIGHTV(1:NOF_VARIABLES))
	...
	ALLOCATE(LEFTV(1:NOF_VARIABLES))
	ALLOCATE(RIGHTV(1:NOF_VARIABLES))
    ALLOCATE(SUMVARS(1:NOF_VARIABLES),MAXVARS(NOF_VARIABLES),aver_vars(nof_Variables))
	ALLOCATe(CORDS(2))
    ...
    allocate(LCVGRAD(nof_Variables-1,dims),RCVGRAD(nof_Variables-1,dims))
    end if

    if ((turbulence.eq.1).or.(passivescalar.gt.0))then
    allocate(LCVGRAD_T(TURBULENCEEQUATIONS+PASSIVESCALAR,dims),RCVGRAD_T(TURBULENCEEQUATIONS+PASSIVESCALAR,dims))
    end if
...
I=1
	...
	ALLOCATE (ILOCAL_ELEM(I)%PERIODICFLAG(M,NUMNEIGHBOURS*iextend))
