...
subroutine cordinates3(n,nodes_list,n_node,cords)
 !> @brief
...
real,dimension(3),intent(out)::cords
real::rnode
real,dimension(1:8,1:dimensiona),intent(in)::nodes_list
  rnode=n_node
 cords(1)=sum(nodes_list(1:n_node,1))/rnode
 cords(2)=sum(nodes_list(1:n_node,2))/rnode
 cords(3)=sum(nodes_list(1:n_node,3))/rnode


end subroutine cordinates3
...
subroutine cordinates2(n,nodes_list,n_node,cords)
  !> @brief
...
#endif
integer,intent(in)::n,n_node
real,dimension(2),intent(out)::cords
real::rnode
real,dimension(1:8,1:dimensiona),intent(in)::nodes_list
  rnode=n_node
 cords(1)=sum(nodes_list(1:n_node,1))/rnode
...
real,dimension(1:gpu_max_dim,1:gpu_max_dim)::vva,vva1
real,dimension(1)::deta
real,dimension(1:4)::vvnxi
real,dimension(1:gpu_max_qp_all)::vvwg
...
real,dimension(1)::deta
real,dimension(1:4)::vvnxi
real,dimension(1:gpu_max_qp_all)::vvwg,vvr1,vvr2,vvr3
...
real,dimension(1:gpu_max_qp_all)::vvwg,vvr1,vvr2,vvr3,vvr4
real,dimension(1:gpu_max_qp_face)::vvwpox,vvnpox,vvwpoy,vvnpoy,vvwpoz,vvnpoz
...
do ii=1,5
  do jj=1,5
    rotvect(ii)=rotvect(ii)+tri(ii,jj)*vectco(jj)
  end do
end do
...
do ii=1,5
  do jj=1,5
    rotvect(ii)=rotvect(ii)+invtri(ii,jj)*vectco(jj)
  end do
end do
