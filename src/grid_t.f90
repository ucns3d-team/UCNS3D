...
SUBROUTINE GEOMETRY_CALC
	...
	!$OMP MASTER
		...
		Moving_TOTALVOLUME(1) = TOTALVOLUME
	!$OMP END MASTER
	!$OMP BARRIER 
...
SUBROUTINE VOLUME_CALCULATOR2(iconsidered)
	...
	allocate(IELEM(N,I)%moving_VOLUME(1+RUNGEKUTTA))
	IELEM(N,I)%moving_VOLUME(:) = 0.0
	...
	IELEM(N,I)%moving_VOLUME(1)=IELEM(N,I)%TOTVOLUME
