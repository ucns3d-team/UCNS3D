...
!$omp declare target(nof_variables,zero,qp_quad,numberofpoints2)
!$omp declare target(qp_triangle)
!$omp declare target(idegfree)
!$omp declare target(IDEGFREE2,IORDER2)
!$omp declare target(ALPHA,BETA)
!$omp declare target(INTEG_BASIS,INTEG_BASIS_DG,VEXT)
!$omp declare target(numneighbours2)
!$omp declare target(OO2)
!$omp declare target(EL_INT)
!$omp declare target(lwcx1)
!$omp declare target(NOF_INTERIOR)
!$omp declare target(XMPIELRANK)
...
integer::jtotAL,JTOTAL1,JTOTAL2,JTOTAL3,compwrt,varcons,pointx,llx,fastmovie
...
REAL::SCHMIDT_TURB
real::PHX,INTEG_MM
real::tolsmall,TOLBIG,allresdt,tz1,DIVBYZERO
...
real::a_rot,c_rot,b_rot,root_rot,sheartemp,LWCX1
...
REAL,ALLOCATABLE,DIMENSION(:,:)::LCVGRAD,RCVGRAD,LCVGRAD_T,RCVGRAD_T
