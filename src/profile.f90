...
SUBROUTINE INITIALISE_EULER2D(N,veccos,pox,poy,poz,iconsidered)
  ...
  !> various customisable profiles can be generated and assigned to each initcond code
    ...
    IF (INITCOND.EQ.403)THEN ! TEST CASE 4.1 OF WANG, DEITERDING, PAN and REN
    ...
    IF (INITCOND.EQ.405)THEN ! TEST CASE 4.5 OF CORALIC & COLONIUS
    ...
    IF (INITCOND.EQ.422)THEN ! TEST CASE 4.5 OF CORALIC & COLONIUS
    ...
    IF (INITCOND.EQ.406)THEN ! TEST CASE 4.5 OF CORALIC & COLONIUS
    ...
    IF (INITCOND.EQ.408)THEN ! TEST CASE 4.5 OF CORALIC & COLONIUS
        IF (POX(1).GT.0.10D0)THEN
            MP_R(1)=6.03
            MP_R(2)=1.658d0
            MP_A(1)=0.0D0
    ...
    IF (INITCOND.EQ.410)THEN ! TEST CASE 6.2 of The Euler Equations for Multiphase Compressible Flow in Conservation Form Simulation of Shock–Bubble Interactions, Robin K. S. Hankin
    ...
    IF (INITCOND.EQ.411)THEN ! EXAMPLE VI Paper5.pdf
    ...
    IF (INITCOND.EQ.412)THEN ! EXAMPLE 4.3 Paper2.pdf
