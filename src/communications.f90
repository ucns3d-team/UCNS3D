...
!$omp& firstprivate(nof_variables, turbulenceequations, passivescalar, turbulence, halos_total, halo_total) &
!$omp& firstprivate(ilength1) &
!$omp& map(alloc: solhi_loc, u_c_val, u_ct_val, solhis, solhir, solhis_flat, solhir_flat) &
!$omp& private(i)
#else
!$omp do
#endif
...
#if defined(gpu) || defined(xpu)
!$omp target data use_device_addr(solhis_flat, solhir_flat)
#endif
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& firstprivate(halos_total) &
!$omp& map(alloc: solhi_loc, ielem_diss, solhisd, solhird)
#else
!$omp do
#endif
do i=1,halos_total
...
#if defined(gpu) || defined(xpu)
!$omp target data use_device_addr(solhisd, solhird)
#endif
...
!$omp target teams distribute parallel do &
!$omp& firstprivate(nof_variables, turbulenceequations, passivescalar, turbulence, halos_total, halo_total) &
!$omp& firstprivate(ilength1, ind1) &
!$omp& map(alloc: solhi_loc, u_c_val, u_ct_val, solhis, solhir, solhis_flat, solhir_flat)
#else
!$omp do
#endif
...
integer::itee,iteedum,jk,jjk,jjk4,jjk12,imulti2,icpe,jmnb,j76,j78,j79,j80,imulti3,i_cnt,cinout2,nvt_comm
integer:: n_requests,baseR,baseS,cell,v
integer, dimension(:), allocatable:: requests
...
nvt_comm=passivescalar
if (turbulence.eq.1) nvt_comm=turbulenceequations+passivescalar

if (dimensiona.eq.3)then


if( itestcase.eq.4)then
i_cnt=(nof_variables+nvt_comm)+(((nof_variables-1)+nvt_comm)*3)
else
...
if( itestcase.eq.4)then
i_cnt=(nof_variables+nvt_comm)+(((nof_variables-1)+nvt_comm)*2)
else
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& firstprivate(nof_variables, turbulenceequations, passivescalar, turbulence, itestcase, dimensiona) &
!$omp& firstprivate(bounds_total, bound_total, ilength2) &
!$omp& map(alloc: need_side, need_q, need_loc, rec_uleft) &
!$omp& map(alloc: boundhis, boundhir, boundhis_flat, boundhir_flat) &
!$omp& private(i)
#else
!$omp do
#endif
...
if (nvt_comm.eq.0)then
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& firstprivate(nof_variables, turbulenceequations, passivescalar, turbulence, itestcase, dimensiona) &
!$omp& firstprivate(bounds_total, bound_total, ilength2, nvt_comm) &
!$omp& map(alloc: need_side, need_q, need_loc, rec_uleft, rec_uleftv) &
!$omp& map(alloc: boundhis, boundhir, boundhis_flat, boundhir_flat) &
!$omp& private(i,ittt,iex,nvar)
#else
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& firstprivate(nof_variables, turbulenceequations, passivescalar, turbulence, itestcase, dimensiona, nvt_comm) &
!$omp& firstprivate(bounds_total, bound_total, ilength2) &
!$omp& map(alloc: need_side, need_q, need_loc, rec_uleft, rec_uleftv, rec_uleftturb, rec_uleftturbv) &
!$omp& map(alloc: boundhis, boundhir, boundhis_flat, boundhir_flat) &
!$omp& private(i,ittt,iex,nvar)
#else
!$omp do private(i,ittt,iex,nvar)
...
#if defined(gpu) || defined(xpu)
!$omp target data use_device_addr(boundhis_flat, boundhir_flat)
#endif
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& firstprivate(nof_variables, turbulenceequations, passivescalar, turbulence, itestcase, relax, iscoun) &
!$omp& firstprivate(dt, bounds_total, bound_total, ilength1) &
!$omp& map(alloc: need_loc, rhs_val, rhst_val, u_c_val, u_ct_val) &
!$omp& map(alloc: impdiag_mf, impdiagt, impdu, ielem_totvolume, boundhisi, boundhiri) &
!$omp& map(alloc: boundhisi_flat, boundhiri_flat)
#else
!$omp do
#endif
do i=1,bounds_total
...
#if defined(gpu) || defined(xpu)
!$omp target data use_device_addr(boundhisi_flat, boundhiri_flat)
#endif
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& firstprivate(nof_variables, turbulenceequations, passivescalar, turbulence, itestcase, bounds_total) &
!$omp& firstprivate(bound_total, ilength1) &
!$omp& map(alloc: need_loc, impdu, boundhisi, boundhiri, boundhisi_flat, boundhiri_flat)
#else
!$omp do
#endif
do i=1,bounds_total
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& firstprivate(bounds_total) &
!$omp& map(alloc: need_loc, ielem_mood, boundhism, boundhirm)
#else
!$omp do
#endif
do i=1,bounds_total
...
#if defined(gpu) || defined(xpu)
!$omp target data use_device_addr(boundhism, boundhirm)
#endif
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& firstprivate(nof_variables, turbulenceequations, passivescalar, itestcase, dims, dimensiona) &
!$omp& firstprivate(bounds_total, bound_total, ilength2) &
!$omp& map(alloc: need_side, need_q, need_loc, rec_uleft_dg, rec_uleftv) &
!$omp& map(alloc: boundhis_dg, boundhir_dg, boundhis_dgflat, boundhir_dgflat)
#else
!$omp do
#endif
    do i=1,bounds_total
...
#if defined(gpu) || defined(xpu)
!$omp target data use_device_addr(boundhis_dgflat, boundhir_dgflat)
#endif
...
!$omp target teams distribute parallel do &
!$omp& firstprivate(nof_variables, turbulenceequations, passivescalar, itestcase, dims, dimensiona) &
!$omp& firstprivate(multispecies, br2_yn, bounds_total, bound_total, ilength2) &
!$omp& map(alloc: need_side, need_q, need_loc) &
!$omp& map(alloc: rec_uleft_dg, rec_uleftv, rec_br2_aux_var, boundhis_dg, boundhir_dg) &
!$omp& map(alloc: boundhis_dgflat, boundhir_dgflat) &
!$omp& private(i,ittt,iex,nvar)
#else
!$omp do private(i,ittt,iex,nvar)
#endif
...
!$omp& firstprivate(multispecies, br2_yn, bounds_total, bound_total, ilength2) &
!$omp& map(alloc: need_side, need_q, need_loc) &
!$omp& map(alloc: rec_uleft_dg, rec_uleftv, boundhis_dg, boundhir_dg) &
!$omp& map(alloc: boundhis_dgflat, boundhir_dgflat)
#else
!$omp do
#endif
...
if (itestcase.eq.4)then
ilength1=(nof_variables+nvt_comm)
ilength2=(nof_variables+nvt_comm)+(((nof_variables-1)+nvt_comm)*dimensiona)
else
...
do i = 1, ineedbounds
  ...
  allocate(need_side(bounds_total),need_q(bounds_total),need_loc(bounds_total))
