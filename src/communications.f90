...
SUBROUTINE EXCHANGE_HIGHER_MOOD(N, RKv)
	!> @brief
	!> This subroutine is exchanging the variables of all the halo cells for all the reconstruction stencils
	IMPLICIT NONE
	INTEGER,INTENT(IN)::N, RKv
	INTEGER::I,J,K,L,M,O,P,Q,INEEDT,TNEEDT,INDL,TNDL,ICPUID,ITEST,ITEE,ITEEDUM,ITEMP1,ITEMP2,IAVC,IAVT
	REAL,DIMENSION(1:1)::DUMTS,RUMTS
	integer:: n_requests
	integer, dimension(:), allocatable:: requests
	...
	If (( turbulence .GT. 0).or.(passivescalar.GT.0)) then
		...
		DO I=1,TNEEDT
			DO K=1,IRECEXS(I)%MUCHTHEYNEED(1)
				  IEXSOLHIS(I)%SOL(K,1:nof_Variables)=U_C(IRECEXS(I)%LOCALREF(K))%VAL(RKv,1:nof_Variables)
				  IEXSOLHIS(I)%SOL(K,NOF_VARIABLES+1:NOF_VARIABLES+TURBULENCEEQUATIONS+PASSIVESCALAR)=U_CT(IRECEXS(I)%LOCALREF(K))%VAL(1,1:TURBULENCEEQUATIONS+PASSIVESCALAR)  
			END DO
		END DO
		!$OMP END DO
		...
		DO I=1,TNEEDT
			DO K=1,IRECEXS(I)%MUCHTHEYNEED(1)
				IEXSOLHIS(I)%SOL(K,1:nof_Variables+TURBULENCEEQUATIONS+PASSIVESCALAR)=U_C(IRECEXS(I)%LOCALREF(K))%VAL(RKv,1:nof_Variables+TURBULENCEEQUATIONS+PASSIVESCALAR)
			END DO
		END DO
		!$OMP END DO
	end if
...
END SUBROUTINE EXCHANGE_HIGHER_MOOD
