...
use transform
implicit none
private :: cordinates2, cordinates3
contains
...
do i = 1, countwallglobal
    ...
    if (transition_model .eq. 1) then
        if (transition_axis .eq. 2) then
            walltr(i) = wally(i) - transition_location
        else if (transition_axis .eq. 3) then
            walltr(i) = wallz(i) - transition_location
        else
            walltr(i) = wallx(i) - transition_location
        end if
    else
        walltr(i) = 0.0
    end if
...
!$omp        wallx,wally,wallz,walltr,xminw,yminw,zminw,dxbin,dybin,dzbin, &
...
do i=1,kmaxe
  if (ielem_interior(i).eq.1)then
	...
	varg_max=max(write_variables_w,write_variables_av_w)
	if (iloopx.gt.0)then
	allocate(sol_vtu_w(1:iloopx,1:varg_max));sol_vtu_w=0	!
	else
	allocate(sol_vtu_w(0:0,1:varg_max));sol_vtu_w=0	!
	end if
	...
	do i=1,iloopx
			if (wall_l(i,3).eq.4)then	!quad
				typ_countn_w=typ_countn_w+4
				typ_nodesn_w(i)=4
				type_vtu_w(i)=9
			end if

			if (wall_l(i,3).eq.3)then	!tri
				typ_countn_w=typ_countn_w+3
				typ_nodesn_w(i)=3
				type_vtu_w(i)=5
			end if

			if (wall_l(i,3).eq.2)then	!line
				typ_countn_w=typ_countn_w+2
				typ_nodesn_w(i)=2
				type_vtu_w(i)=3
			end if
	end do
...
!$omp& map(alloc: ibound_icode, ielem_faceanglex, ielem_faceangley, ielem_ggs, ielem_ibounds, ielem_ifca) &
!$omp& map(alloc: ielem_interior, ielem_surf, ielem_types_faces, ielem_xxc, ielem_yyc, ielem_zzc, origin) &
!$omp& map(alloc: rec_grads, rec_mrf, rec_uleft, rec_uleft_dg, rec_uleftv, weights_q, weights_t) &
!$omp& firstprivate(dg, itestcase, mrf, nof_variables, pres, qp_quad, qp_triangle, zero)
#else
...
!$omp& map(alloc: ibound_icode, ielem_faceanglex, ielem_faceangley, ielem_ggs, ielem_ibounds, ielem_ifca) &
!$omp& map(alloc: ielem_interior, ielem_surf, rec_grads, rec_uleft, rec_uleftv, weights_l) &
!$omp& firstprivate(itestcase, nof_variables, pres, qp_line, qp_line_n, zero)
#else
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& firstprivate(k) private(i) reduction(+:res_sum) &
!$omp& map(alloc: ielem_totvolume, rhs_val, rhs_valdg) &
!$omp& firstprivate(dg)
#else
!$omp do reduction(+:res_sum)
#endif
do i=1,kmaxe
	if (dg.eq.1)then
...
!$omp target teams distribute parallel do &
!$omp& firstprivate(k) private(i) reduction(+:res_sum) &
!$omp& map(alloc: ielem_totvolume, rhs_val, rhst_val) &
!$omp& firstprivate(nof_variables)
#else
!$omp do reduction(+:res_sum)
#endif
do i=1,kmaxe
...
!$omp target teams distribute parallel do &
!$omp& firstprivate(k) private(i) reduction(+:res_sum) &
!$omp& map(alloc: ielem_totvolume, rhs_val) &
!$omp& firstprivate(dg)
#else
!$omp do reduction(+:res_sum)
#endif
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& private(i) reduction(+:ipos_l1,ipos_l2) &
!$omp& map(alloc: ielem_mood_o) &
!$omp& firstprivate(iorder)
#else
!$omp do reduction(+:ipos_l1,ipos_l2)
#endif
do i=1,kmaxe
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& private(i) reduction(+:pos_l1) &
!$omp& map(alloc: ielem_condition) &
!$omp& firstprivate(kmaxe)
#else
!$omp do reduction(+:pos_l1)
#endif
do i=1,kmaxe
    pos_l1=pos_l1+ielem_condition(i)
end do
#if defined(gpu) || defined(xpu)
!$omp end target teams distribute parallel do
#else
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& private(i,tempint) reduction(+:pos_l1) &
!$omp& map(alloc: ielem_reduce) &
!$omp& firstprivate(kmaxe)
#else
!$omp do private(tempint) reduction(+:pos_l1)
#endif
do i=1,kmaxe
    tempint=0
...
if (n.eq.0)then
    ...
    write(70,'(e14.7,1x,e14.7)')t,(pos_g(1)/imaxe)*100.0
    close(70)
end if
...
#if defined(gpu) || defined(xpu)
!$omp target teams distribute parallel do &
!$omp& private(i) reduction(+:pos_l1,pos_l2,pos_l3,pos_l4) &
!$omp& map(alloc: ielem_er, ielem_er1, ielem_er2, ielem_filtered) &
!$omp& firstprivate(kmaxe)
#else
!$omp do reduction(+:pos_l1,pos_l2,pos_l3,pos_l4)
#endif
do i=1,kmaxe
    pos_l1=pos_l1+ielem_filtered(i)
    pos_l3=pos_l3+ielem_er(i)
    pos_l2=pos_l2+ielem_er1(i)
    pos_l4=pos_l4+ielem_er2(i)
end do
#if defined(gpu) || defined(xpu)
!$omp end target teams distribute parallel do
#else
