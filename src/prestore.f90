...
character*50 filename_invmat_stencilt,filename_lscqmat
	...
	! write(N_string, '(I3)') N
	! filename_invmat_stencilt = 'ConditionNumber_invmat_stencilt'//TRIM(ADJUSTL(N_string))//'.dat'
	! filename_lscqmat = 'ConditionNumber_lsqcmat'//TRIM(ADJUSTL(N_string))//'.dat'
	! open(7000+N, file = filename_invmat_stencilt)
	! open(8000+N, file = filename_lscqmat)
	! if (dimensiona == 2) then
	! 	write(7000+N,*) 'x y condition_number max_eigv min_eigv'
	! 	write(8000+N,*) 'x y condition_number max_eigv min_eigv'
	! else
	! 	write(7000+N,*) 'x y z condition_number max_eigv min_eigv'
	! 	write(8000+N,*) 'x y z condition_number max_eigv min_eigv'
	! end if
...
real,allocatable,dimension(:,:):: matrix_copy_u,matrix_copy_v
real,allocatable,dimension(:):: svd_diagonal
real::min_eigv,max_eigv,condition_number
integer::iter
...
! allocate(matrix_copy_u(1:idegfree,1:idegfree),matrix_copy_v(1:idegfree,1:idegfree))
! allocate(svd_diagonal(1:idegfree))
...
! allocate(matrix_copy_u(1:idegfree,1:100),matrix_copy_v(1:100,1:100))
! allocate(svd_diagonal(1:100))
...
DO LL=1,LLCO	!ADMIS
	...
	! write(8000+N, '(G0, X, G0, X, G0, X, G0, X, G0)') IELEM(N,i )%XXC, IELEM(N,i)%YYC, condition_number, max_eigv, min_eigv
	...
	! write(7000+N, '(G0, X, G0, X, G0, X, G0, X, G0)') IELEM(N,i )%XXC, IELEM(N,i)%YYC, condition_number, max_eigv, min_eigv
