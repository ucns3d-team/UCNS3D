	 ...
	 DO j=1,IELEM(N,I)%IFCA		!for all faces
	    if (ielem(n,i)%ibounds(J).gt.0)then		!for bounded only
	      if (ibound(n,ielem(n,i)%ibounds(j))%icode.eq.4)then		!!for bounded only 2                         
				...
				END IF
				IF (POLY.EQ.4) THEN
				XDER(IQ)=TL3DX(AX,AY,AZ,IQ,I,IELEM(N,I)%totvolume);  YDER(IQ)=TL3DY(AX,AY,AZ,IQ,I,IELEM(N,I)%totvolume);  ZDER(IQ)=TL3DZ(AX,AY,AZ,IQ,I,IELEM(N,I)%totvolume)
				END IF
	 ...
	 DO j=1,IELEM(N,I)%IFCA		!for all faces
	    if (ielem(n,i)%ibounds(J).gt.0)then		!for bounded only
	      if (ibound(n,ielem(n,i)%ibounds(j))%icode.eq.4)then		!!for bounded only 2                         
				...
				XDER(IQ)=TL2dX(AX,AY,IQ,I,IELEM(N,I)%totvolume);  YDER(IQ)=TL2dY(AX,AY,IQ,I,IELEM(N,I)%totvolume);
...
WEFF=zero
 	...
 	DO I=1,IDEG
			...
			INTEG =zero
				...
				PH=TL3DX(QPOINTS(1,K),QPOINTS(2,K),QPOINTS(3,K),I,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume)*TL3DX(QPOINTS(1,K),QPOINTS(2,K),QPOINTS(3,K),J,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume) + &
				   TL3DY(QPOINTS(1,K),QPOINTS(2,K),QPOINTS(3,K),I,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume)*TL3DY(QPOINTS(1,K),QPOINTS(2,K),QPOINTS(3,K),J,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume) + &
				   TL3DZ(QPOINTS(1,K),QPOINTS(2,K),QPOINTS(3,K),I,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume)*TL3DZ(QPOINTS(1,K),QPOINTS(2,K),QPOINTS(3,K),J,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume)
...
WEFF=zero
 	...
 	DO I=1,IDEG
			...
			INTEG =zero
				...
				PH=TL2DX(QPOINTS(1,K),QPOINTS(2,K),I,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume)*TL2DX(QPOINTS(1,K),QPOINTS(2,K),J,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume) + &
				   TL2DY(QPOINTS(1,K),QPOINTS(2,K),I,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume)*TL2DY(QPOINTS(1,K),QPOINTS(2,K),J,ICONSIDERED,IELEM(N,ICONSIDERED)%totvolume)
