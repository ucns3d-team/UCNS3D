					...
					if (ibx1.eq.4)then
						if ((inoder(IB_n(1))%itor.gt.0).and.(inoder(IB_n(2))%itor.GT.0))then
							...
							DO IBLEED=1,BLEED_NUMBER
								IF (((inoder(IB_n(1))%CORD(1).GE.bleed_start(IBLEED,1)).AND.&
									 (inoder(IB_n(1))%CORD(1).LE.bleed_END(IBLEED,1))).AND.&
									 ((inoder(IB_n(2))%CORD(1).GE.bleed_start(IBLEED,1)).AND.&
									 (inoder(IB_n(2))%CORD(1).LE.bleed_END(IBLEED,1))).AND.&
									 ((inoder(IB_n(1))%CORD(2).GE.bleed_start(IBLEED,2)).AND.&
									 (inoder(IB_n(1))%CORD(2).LE.bleed_END(IBLEED,2))).AND.&
									 ((inoder(IB_n(2))%CORD(2).GE.bleed_start(IBLEED,2)).AND.&
									 (inoder(IB_n(2))%CORD(2).LE.bleed_END(IBLEED,2)))) THEN
				...
				IF (ielem(n,i)%nofbc.GT.0)THEN		! THAT HAVE AT LEAST ESTABLISHED A BOUNDARY CONDITION CODE
					DO J=1,ielem(n,i)%IFCA			! LOOP ALL THEIR BOUNDARY FACES
						if (IELEM(N,I)%IBOUNDS(J).gt.0)then
							...
							IF (BLEED.EQ.1)THEN
								IF (IBOUND(N,IELEM(N,I)%IBOUNDS(J))%ICODE.EQ.99) THEN ! ASSIGN THE BLEED ZONE
									N_NODE=2
									DO Kk=1,n_node
