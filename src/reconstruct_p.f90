...
SUBROUTINE EXTRAPOLATE_BOUND_SPHERE_CASE_ONLY(varcons, FACEX, pointx, ICONSIDERED, INSTEN, LLX, WENO_L, RESSOLUTION_L, U_C_L, ILOCAL_RECON3_L)
	!> @brief
	!> Subroutine for extrapolating the reconstructed solution at the cell interfaces
	   ...
	   REAL,DIMENSION(:,:),INTENT(INOUT)::WENO_L,RESSOLUTION_L
	   TYPE(U_CENTRE),DIMENSION(:),INTENT(IN)::U_C_L
	   TYPE(LOCAL_RECON3),DIMENSION(:),INTENT(INOUT)::ILOCAL_RECON3_L
		  ...
		  ILOCAL_RECON3_L(ICONSIDERED)%ULEFT(1:NOF_VARIABLES,FACEX,pointx)=ILOCAL_RECON3_L(ICONSIDERED)%ULEFT(1:NOF_VARIABLES,FACEX,pointx)&
						  + (U_C_L(ICONSIDERED)%VAL(1, 1:NOF_VARIABLES) + RESSOLUTION_L(INSTEN, 1:NOF_vARIABLES))*WENO_L(1:NOF_vARIABLES, LLX)
...
SUBROUTINE WENOWEIGHTS_KERNEL_SPHERE_CASE_ONLY(N, UGRADLOC_L, GRAD1AL_L, INDICATEMATRIXAL_L, compwrt_L, imax_L, NUMBER_OF_DOG_L, &
	 NUMBER_OF_NEI_L, ANGLE1_L, ANGLE2_L, AX_L, AY_L, AZ_L,SB_L, CONSMATRIX_L, CONSMATRIXC_L, WENO_L, RESSOLUTION_L, &
	...
	REAL,DIMENSION(:,:),INTENT(INOUT)::CONSMATRIX_L, CONSMATRIXC_L, WENO_L, RESSOLUTION_L
	...
	REAL,DIMENSION(:),INTENT(INOUT):: vvwg_L, VVR1_L, VVR2_L, VVR3_L, vvnpox_L, vvnpoy_L, vvnpoz_L, vvwpox_L, vvwpoy_L, vvwpoz_L, VVnxi_L

	REAL,DIMENSION(:),INTENT(INOUT):: lamc_L, LAMBDAAL_L, GRAD3AL_L
	REAL,DIMENSION(:,:),INTENT(INOUT):: GRAD5ALc_L
	REAL,DIMENSION(:),INTENT(INOUT):: SMOOTHINDICATORAL_L, OMEGAATILDEL_L, OMEGAAL_L
	REAL,DIMENSION(:,:),INTENT(INOUT):: GRADSSL_L
...
END SUBROUTINE WENOWEIGHTS_KERNEL_SPHERE_CASE_ONLY
...
SUBROUTINE SOLUTIONTRIAV2_SPHERE_CASE_ONLY(N, ICONSIDERED, AX_L, AY_L, AZ_L, GRADSS_L, AINVJT_L, WEIGHT_T2_L, XXDER_L, YYDER_L, ZZDER_L, GRADTEM_L, XDER_L, YDER_L, ZDER_L, UGRADLOC_L,WEQUA2D_L,QPOINTS2D_L,vvwg_L,VVR1_L,VVR2_L,VVR3_L,vvnpox_L,vvnpoy_L,vvnpoz_L,vvwpox_L,vvwpoy_L,vvwpoz_L,VVnxi_L, IELEM_L, ILOCAL_RECON3_L, ILOCAL_RECON5_L)
	!> @brief
	!> Subroutine for extrapolating the unlimited reconstructed values for diffusive fluxes in 3D
	   ...
	   call QUADRATUREQUAD3D_SPHERE_CASE_ONLY(N,IGQRULES,WEQUA2D_L,QPOINTS2D_L,vvwg_L,VVR1_L,VVR2_L,vvnpox_L,vvnpoy_L,vvnpoz_L,vvwpox_L,vvwpoy_L,vvwpoz_L,VVnxi_L); WEIGHTS_Q(1:QP_QUAD) = WEQUA2D_L(1:QP_QUAD)
	   call QUADRATURETRIANG_SPHERE_CASE_ONLY(N,IGQRULES,WEQUA2D_L,QPOINTS2D_L,vvwg_L,VVR1_L,VVR2_L,VVR3_L); WEIGHTS_T(1:QP_TRIANGLE) = WEQUA2D_L(1:QP_TRIANGLE)
	   ...
	   DO IHGT = 1, 3; DO IHGJ = 1, 3
			 AINVJT_L(IHGT, IHGJ) = ILOCAL_RECON3_L(I)%INVCCJAC(IHGJ, IHGT)
		  END DO; END DO
...
!-----------------
	   ...
	   DO l = 1, IELEM_L(N, I)%IFCA; IDUMMY = 0
		  ...
		  do NGP = 1, iqp                        !for gqp
			 ...
			 AX_L = ILOCAL_RECON3_L(I)%QPOINTS(L, NGP, 1); AY_L = ILOCAL_RECON3_L(I)%QPOINTS(L, NGP, 2); AZ_L = ILOCAL_RECON3_L(I)%QPOINTS(L, NGP, 3)
			 icd = icd + 1
			 DO K = 1, IELEM_L(N, I)%IDEGFREE
				XXDER_L(K, ICD) = TL3DX_SPHERE_CASE_ONLY(I, AX_L, AY_L, AZ_L, K, IELEM_L); YYDER_L(K, ICD) = TL3DY_SPHERE_CASE_ONLY(I, AX_L, AY_L, AZ_L, K, IELEM_L); ZZDER_L(K, ICD) = TL3DZ_SPHERE_CASE_ONLY(I, AX_L, AY_L, AZ_L, K, IELEM_L)
			 END DO
		  end do
		  ICD = 0
		  ...
		  do NGP = 1, iqp
			 ...
			 CASE (0)
				...
				GRADTEM_L(1:IELEM_L(N, I)%IDEGFREE) = ILOCAL_RECON5_L(I)%GRADIENTSTEMP(1:IELEM_L(N, I)%IDEGFREE)
	!
				UGRADLOC_L = ZERO
	
				UGRADLOC_L(1) = DDOT_D(IELEM_L(N, I)%IDEGFREE, GRADTEM_L(1:IELEM_L(N, I)%IDEGFREE), 1, XXDER_L(1:IELEM_L(N, I)%IDEGFREE, ICD), 1)
				UGRADLOC_L(2) = DDOT_D(IELEM_L(N, I)%IDEGFREE, GRADTEM_L(1:IELEM_L(N, I)%IDEGFREE), 1, YYDER_L(1:IELEM_L(N, I)%IDEGFREE, ICD), 1)
				UGRADLOC_L(3) = DDOT_D(IELEM_L(N, I)%IDEGFREE, GRADTEM_L(1:IELEM_L(N, I)%IDEGFREE), 1, ZZDER_L(1:IELEM_L(N, I)%IDEGFREE, ICD), 1)
	
				ILOCAL_RECON3_L(I)%ULEFTV(1:3, 1, L, NGP) = MATMUL(AINVJT_L(1:3, 1:3), UGRADLOC_L(1:3))
	
				DO IEX = 1, 3
	!
				   GRADTEM_L(1:IELEM_L(N, I)%IDEGFREE) = ILOCAL_RECON5_L(I)%VELOCITYDOF(IEX, 1:IELEM_L(N, I)%IDEGFREE)
	!
				   UGRADLOC_L = ZERO
				   ...
				   ILOCAL_RECON3_L(I)%ULEFTV(1:3, IEX + 1, L, NGP) = MATMUL(AINVJT_L(1:3, 1:3), UGRADLOC_L(1:3))
	...
	!
			 ...
			 CASE (1)
				...
				ILOCAL_RECON3_L(I)%ULEFTV(1:3, 1, L, NGP) = ILOCAL_RECON3_L(I)%GRADs(4, 1:3)
	
				DO IEX = 1, 3
				   ILOCAL_RECON3_L(I)%ULEFTV(1:3, IEX + 1, L, NGP) = ILOCAL_RECON3_L(I)%GRADs(IEX, 1:3)
				END DO
	
			 CASE (2)
				DO K = 1, IELEM_L(N, I)%IDEGFREE
				   GRADTEM_L(K) = ILOCAL_RECON5_L(I)%GRADIENTSTEMP(K)
				   XDER_L(K) = TL3DX_SPHERE_CASE_ONLY(I, AX_L, AY_L, AZ_L, K, IELEM_L); YDER_L(K) = TL3DY_SPHERE_CASE_ONLY(I, AX_L, AY_L, AZ_L, K, IELEM_L); ZDER_L(K) = TL3DZ_SPHERE_CASE_ONLY(I, AX_L, AY_L, AZ_L, K, IELEM_L)
				END DO
				UGRADLOC_L = ZERO
				DO K = 1, IDEGFREE
				...
				DO IEX = 1, 3
				   DO K = 1, IELEM_L(N, I)%IDEGFREE
					  GRADTEM_L(K) = ILOCAL_RECON5_L(I)%VELOCITYDOF(IEX, K)
