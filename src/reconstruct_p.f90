...
TYPE(A_INTEGRALBASIS),ALLOCATABLE,DIMENSION(:),INTENT(INOUT)::INTEG_BASIS_L,integ_basis_dg_L
TYPE(A_NODE_NE),ALLOCATABLE,DIMENSION(:),INTENT(INOUT)::INODER4_L
TYPE(A_EXCHANGE_SOLHI),ALLOCATABLE,DIMENSION(:),INTENT(INOUT)::IEXSOLHIR_L
TYPE(A_VOL_GQP),ALLOCATABLE,DIMENSION(:),INTENT(INOUT)::QP_ARRAY_L
...
TYPE(A_device_packed_parameters) :: dparams
...
SUBROUTINE pack_params_for_device(dparams)
        IMPLICIT NONE
        !$OMP declare target
        TYPE(A_device_packed_parameters),INTENT(INOUT) :: dparams
        dparams%N = N
