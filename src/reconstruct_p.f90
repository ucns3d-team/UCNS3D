  ...
  !> Subroutine for storing the gaussian quadrature points at the cell interfaces
    ...
    if (dimensiona.eq.3)then
        ...
        DO I=1,KMAXE
            ...
            ALLOCATE(ILOCAL_RECON3(I)%QPOINTS_velocity(IELEM(N,I)%IFCA,qp_line,2))
...
Subroutine Allocate_QP_points(N)
  ! allocates memory for quadrature points
  ! copied from the original version of MEMORY_FAST
    implicit none
    integer, intent(in):: n
    integer::i, kmaxe
...
end subroutine Allocate_QP_points
! ! !---------------------------------------------------------------------------------------------!
