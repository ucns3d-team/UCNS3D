...
!----------------REAL GAS COMMENTS
THINGS TO VALIDATE THE IMPLEMENTATION:

1) MODIFY MULTISPECIES/NOF_SPECIES FOR DIFFUSION AS WELL AND NOF-VARIABLES (TT)-1h   !completed the gradf variable

2) RESTRUCTURE LSQ-DIRICHLET-NEUMANN  (TT)-1h !done

3) INVESTIGATE WHICH BC'S WE MOST LIKELY WANT FOR THE WALL (ISOTHERMAL (WORKS-EASY), CATALYTIC ETC)- (WP)	!done

4) CONS2PRIM AND PRIM2CONS MODIFY TO BE MORE GENERIC (TEMPERATURE VECTOR TO BE INCLUDED) (TT)-2h !partly done

5) SOURCE REAL GAS CHECK IMPLEMENTATION (WP)!yes done

6) OUTPUT ROTATIONAL-TRANSLATIONAL AND VIBRATIONAL TEMPERATURE (TT)-1h	!conv2dif -partly done

7) RIEMANN FIX HLLC/HLL/RUSANOV FOR REAL GAS (TT)-1h		!to be extended

8) SPECIFY WHAT VARIABLES TO EXPORT (WP) FOR ANALYSIS 1h		!to be done

9) memory allocation check in the parameters for global variables 0.5


10)check the gradients one more time
