...
subroutine riemann_flux_ideal3d(n,iconsidered,facex,cleft_rot,cright_rot,hllcflux,srf_speedrot)
implicit none
...
real,dimension(1:gpu_max_nvar_total)::fl,fr,ulstar,urstar,flstar,frstar,fhll
real::mp_pinfl,mp_pinfr,gammal,gammar
real::rl,rr,ul,ur,vl,vr,wl,wr,pl,pr,el,er,ccl,ccr,cc2,uu2,sl,sr,sm
real::mul,mur,lastl,lastr,lam
integer::iv,nvt
...
rl=leftv(1); ul=leftv(2); vl=leftv(3); wl=leftv(4); pl=leftv(5); el=cleft_rot(5)
rr=rightv(1); ur=rightv(2); vr=rightv(3); wr=rightv(4); pr=rightv(5); er=cright_rot(5)
...
if (nvt.gt.0)then
  do iv=1,nvt
    fl(nof_variables+iv)=cleft_rot(nof_variables+iv)*ul
    fr(nof_variables+iv)=cright_rot(nof_variables+iv)*ur
  end do
end if
...
if (iriemann.eq.9)then
  ...
  sr=max(ul+ccl,ur+ccr)
  do iv=1,nof_variables+nvt
    fhll(iv)=(sr*fl(iv)-sl*fr(iv)+(sl*sr*(cright_rot(iv)-cleft_rot(iv))))/(sr-sl)
  end do
  if (sl.ge.zero)then
    hllcflux(1:nof_variables+nvt)=fl(1:nof_variables+nvt)
  else if (sr.le.zero)then
    hllcflux(1:nof_variables+nvt)=fr(1:nof_variables+nvt)
  else
    hllcflux(1:nof_variables+nvt)=fhll(1:nof_variables+nvt)
  end if
  return
end if
...
mul=rl*((sl-ul)/(sl-sm))
mur=rr*((sr-ur)/(sr-sm))
lastl=(el/rl)+((sm-ul)*(sm+(pl/(rl*(sl-ul)))))
lastr=(er/rr)+((sm-ur)*(sm+(pr/(rr*(sr-ur)))))
ulstar(1)=mul
ulstar(2)=mul*sm
ulstar(3)=mul*vl
ulstar(4)=mul*wl
...
if (nvt.gt.0)then
  do iv=1,nvt
    ulstar(nof_variables+iv)=mul*cleft_rot(nof_variables+iv)/rl
    urstar(nof_variables+iv)=mur*cright_rot(nof_variables+iv)/rr
  end do
end if
do iv=1,nof_variables+nvt
  flstar(iv)=fl(iv)+sl*(ulstar(iv)-cleft_rot(iv))
  frstar(iv)=fr(iv)+sr*(urstar(iv)-cright_rot(iv))
end do
...
if (rec_mrf(iconsidered).eq.0)then
  if (sl.ge.zero)then
    ...
    hllcflux(1:nof_variables+nvt)=fr(1:nof_variables+nvt)
  else if (sm.ge.zero)then
    hllcflux(1:nof_variables+nvt)=flstar(1:nof_variables+nvt)
  else
    hllcflux(1:nof_variables+nvt)=frstar(1:nof_variables+nvt)
  end if
else
  if ((sl-srf_speedrot(2)).ge.zero)then
    hllcflux(1:nof_variables+nvt)=fl(1:nof_variables+nvt)-srf_speedrot(2)*cleft_rot(1:nof_variables+nvt)
  else if ((sr-srf_speedrot(2)).le.zero)then
    hllcflux(1:nof_variables+nvt)=fr(1:nof_variables+nvt)-srf_speedrot(2)*cright_rot(1:nof_variables+nvt)
  else if ((sm-srf_speedrot(2)).ge.zero)then
    hllcflux(1:nof_variables+nvt)=flstar(1:nof_variables+nvt)-srf_speedrot(2)*ulstar(1:nof_variables+nvt)
  else
    hllcflux(1:nof_variables+nvt)=frstar(1:nof_variables+nvt)-srf_speedrot(2)*urstar(1:nof_variables+nvt)
  end if
end if

end subroutine riemann_flux_ideal3d


subroutine riemann_flux_ideal2d(n,cleft_rot,cright_rot,hllcflux,srf_speedrot)
implicit none
#if defined(gpu) || defined(xpu)
!$omp declare target
#endif
...
real,dimension(1:gpu_max_nvar_total)::fl,fr,ulstar,urstar,flstar,frstar,fhll
real::mp_pinfl,mp_pinfr,gammal,gammar
real::rl,rr,ul,ur,vl,vr,pl,pr,el,er,ccl,ccr,cc2,uu2,sl,sr,sm
real::mul,mur,lastl,lastr,lam
integer::iv,nvt
...
rl=leftv(1); ul=leftv(2); vl=leftv(3); pl=leftv(4); el=cleft_rot(4)
rr=rightv(1); ur=rightv(2); vr=rightv(3); pr=rightv(4); er=cright_rot(4)
ccl=sqrt(gamma*pl/rl)
ccr=sqrt(gamma*pr/rr)
...
end subroutine riemann_flux_ideal2d
...
!$omp declare target
	...
	real,dimension(1:nof_variables+turbulenceequations+passivescalar),intent(in)::cleft_rot,cright_rot
	real,dimension(1:nof_variables),intent(in)::srf_speedrot
	real,dimension(1:gpu_max_nvar_total)::rotvl,rotvr
	real,dimension(1:gpu_max_nvar)::leftv,rightv
	real,dimension(1:nof_variables+turbulenceequations+passivescalar),intent(inout)::hllcflux
	...
	real,dimension(1:gpu_max_nvar_total)::tempfl,tempfr
	real,dimension(1:gpu_max_nvar_total)::flstar,frstar
	real,dimension(1:gpu_max_nvar_total)::ulstar,urstar
	real,dimension(1:gpu_max_nvar_total)::tempul,tempur
	real,dimension(1:gpu_max_nvar_total)::fl,fr
	real,dimension(gpu_max_extra_transport)::rml,rmr
	real,dimension(1:gpu_max_nvar_total)::fhll
...
! 			end if
        ...
        sr(1)=max(ul+ccl,ur+ccr)
			...
			fhll(1:nof_variables+turbulenceequations+passivescalar)=(sr(1)*fl(1:nof_variables+turbulenceequations+passivescalar)-sl(1)*fr(1:nof_variables+turbulenceequations+passivescalar)+(sl(1)*sr(1)*(cright_rot(1:nof_variables+turbulenceequations+passivescalar)-cleft_rot(1:nof_variables+turbulenceequations+passivescalar))))/(sr(1)-sl(1))

			

			
			if (sl(1).ge.zero)then
				hllcflux(1:nof_variables+turbulenceequations+passivescalar)=fl(1:nof_variables+turbulenceequations+passivescalar)
			...
			if (sr(1).le.zero)then
				hllcflux(1:nof_variables+turbulenceequations+passivescalar)=fr(1:nof_variables+turbulenceequations+passivescalar)
			...
			if ((sl(1).le.zero).and.(sr(1).ge.zero))then
				hllcflux(1:nof_variables+turbulenceequations+passivescalar)=fhll(1:nof_variables+turbulenceequations+passivescalar)
...
! 			end if
	...
	sl(1)=min(ul-ccl,uu2-cc2); sr(1)=max(ur+ccr,uu2+cc2)
		...
		sm(1)=(pr-pl+(rl*ul*(sl(1)-ul))-(rr*ur*(sr(1)-ur)))/((rl*(sl(1)-ul))-(rr*(sr(1)-ur)))
			...
			flstar(1:nof_variables+turbulenceequations+passivescalar)=fl(1:nof_variables+turbulenceequations+passivescalar)+sl(1)*(ulstar(1:nof_variables+turbulenceequations+passivescalar)-cleft_rot(1:nof_variables+turbulenceequations+passivescalar))
			frstar(1:nof_variables+turbulenceequations+passivescalar)=fr(1:nof_variables+turbulenceequations+passivescalar)+sr(1)*(urstar(1:nof_variables+turbulenceequations+passivescalar)-cright_rot(1:nof_variables+turbulenceequations+passivescalar))
			...
			(((1.0d0-sign(1.0d0,sm(1)))/2.0d0)*(fr(1:nof_variables+turbulenceequations+passivescalar)+sr(1)*(urstar(1:nof_variables+turbulenceequations+passivescalar)-cright_rot(1:nof_variables+turbulenceequations+passivescalar))))
                ...
                if ((sl(1)-srf_speedrot(2)).ge.zero)then
                        hllcflux(1:nof_variables+turbulenceequations+passivescalar)=fl(1:nof_variables+turbulenceequations+passivescalar)-srf_speedrot(2)*cleft_rot(1:nof_variables+turbulenceequations+passivescalar)
                end if
                if ((sr(1)-srf_speedrot(2)).le.zero)then
                        hllcflux(1:nof_variables+turbulenceequations+passivescalar)=fr(1:nof_variables+turbulenceequations+passivescalar)-srf_speedrot(2)*cright_rot(1:nof_variables+turbulenceequations+passivescalar)
                end if
                if (((sl(1)-srf_speedrot(2)).le.zero).and.((sm(1)-srf_speedrot(2)).ge.zero))then
                        hllcflux(1:nof_variables+turbulenceequations+passivescalar)=flstar(1:nof_variables+turbulenceequations+passivescalar)-srf_speedrot(2)*ulstar(1:nof_variables+turbulenceequations+passivescalar)
                end if
                if (((sr(1)-srf_speedrot(2)).ge.zero).and.((sm(1)-srf_speedrot(2)).le.zero))then
                        hllcflux(1:nof_variables+turbulenceequations+passivescalar)=frstar(1:nof_variables+turbulenceequations+passivescalar)-srf_speedrot(2)*urstar(1:nof_variables+turbulenceequations+passivescalar)
                end if
            end if
...
!dissipation term: |an|(ur-ul) = r|lambda|l*du = sum_k of [ ws(k) * r(:,k) * l*du(k) ]
 ...
 diss(1:nof_variables) = ws(1)*ldu(1)*r(1:nof_variables,1) + ws(2)*ldu(2)*r(1:nof_variables,2) &
         + ws(3)*ldu(3)*r(1:nof_variables,3) + ws(4)*ldu(4)*r(1:nof_variables,4)
...
!dissipation term: |an|(ur-ul) = r|lambda|l*du = sum_k of [ ws(k) * r(:,k) * l*du(k) ]
 ...
 diss(1:nof_variables) = ws(1)*ldu(1)*r(1:nof_variables,1) + ws(2)*ldu(2)*r(1:nof_variables,2) + ws(3)*ldu(3)*r(1:nof_variables,3) &
         + ws(4)*ldu(4)*r(1:nof_variables,4) + ws(5)*ldu(5)*r(1:nof_variables,5)
