			 ...
			 DO L=1,IELEM(N,I)%IFCA	!faces2
			  ILOCAL_RECON3(I)%ULEFTTURB(:,L,:)=zero
			      ...
			      do NGP=1,iqp
				    ...
				    if (reduce_comp.eq.1)then
				    ILOCAL_RECON3(I)%ULEFTTURB(1:TURBULENCEEQUATIONS+PASSIVESCALAR,L,1)=ILOCAL_RECON3(I)%ULEFTTURB(1:TURBULENCEEQUATIONS+PASSIVESCALAR,L,1)+&
				    (U_CT(I)%VAL(1,1:TURBULENCEEQUATIONS+PASSIVESCALAR)+(USOL(1:TURBULENCEEQUATIONS+PASSIVESCALAR,l,Ngp)*WENO(1:TURBULENCEEQUATIONS+PASSIVESCALAR,1)))*wequa2d(ngp)
				    else
				    ILOCAL_RECON3(I)%ULEFTTURB(1:TURBULENCEEQUATIONS+PASSIVESCALAR,L,ngp)=U_CT(I)%VAL(1,1:TURBULENCEEQUATIONS+PASSIVESCALAR)+(USOL(1:TURBULENCEEQUATIONS+PASSIVESCALAR,l,Ngp)&
				    *WENO(1:TURBULENCEEQUATIONS+PASSIVESCALAR,1))
