...
!         WRITE(400+N,*) "DT", DT
!         WRITE(400+N,*) "INV_MASS_MATRIX", INV_MASS_MATRIX(N,I,:,:)
!         WRITE(400+N,*) "RHS(I)%VALDG(:,1:NOF_VARIABLES)", RHS(I)%VALDG(:,1:NOF_VARIABLES)   
!         WRITE(400+N,*) "TRANSPOSE", TRANSPOSE(MATMUL(INV_MASS_MATRIX(N,I,:,:), RHS(I)%VALDG(:,1:NOF_VARIABLES)))
!         WRITE(400+N,*) "U_C(I)%VALDG(2,1:NOF_VARIABLES,:)", U_C(I)%VALDG(2,1:NOF_VARIABLES,:)
!         WRITE(400+N,*) "U_C(I)%VALDG(1,1:NOF_VARIABLES,:)", U_C(I)%VALDG(1,1:NOF_VARIABLES,:)
    ELSE
        OOVOLUME=1.0D0/IELEM(N,I)%TOTVOLUME
        U_C(I)%VAL(2,1:NOF_VARIABLES)=U_C(I)%VAL(1,1:NOF_VARIABLES)
        U_C(I)%VAL(1,1:NOF_VARIABLES)=U_C(I)%VAL(2,1:NOF_VARIABLES)-(DT*(RHS(I)%VAL(1:NOF_VARIABLES)*OOVOLUME))
...
IF (DG == 1 .AND. ANY(U_C(1)%VALDG(1,:,:) /= U_C(1)%VALDG(1,:,:))) THEN
...
DO I=1,KMAXE
    ...
    IF (DG == 1) THEN
        ...
        U_C(I)%VALDG(1,:,:) = 0.178079954393132 * U_C(I)%VALDG(2,:,:) + 0.821920045606868 * U_C(I)%VALDG(5,:,:) + 0.544974750228521 * DT * U_C(I)%VALDG(6,:,:)! * OOVOLUME
    ELSE
        U_C(I)%VAL(5,1:NOF_VARIABLES) = U_C(I)%VAL(1,1:NOF_VARIABLES)
        U_C(I)%VAL(6,1:NOF_VARIABLES) = - DT * RHS(I)%VAL(1:NOF_VARIABLES) * OOVOLUME
