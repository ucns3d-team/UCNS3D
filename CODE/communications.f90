...
DO I=1,TNEEDT
	DO K=1,IRECEXS(I)%MUCHTHEYNEED(1)
            IF (DG == 1) THEN
                ...
                IEXSOLHIS(I)%DELTA_XYZ(K,:) = IELEM( IELEM(N,I)%INEIGHB(K), IRECEXS(I)%LOCALREF(K) )%DELTA_XYZ
                X1 = IELEM(IELEM(N,I)%INEIGHB(K), IRECEXS(I)%LOCALREF(K))%XXC
                Y1 = IELEM(IELEM(N,I)%INEIGHB(K), IRECEXS(I)%LOCALREF(K))%YYC
                IEXSOLHIS(I)%BASIS_NEIGHBOR_CENTER(K,:) = BASIS_REC2D(IELEM(N,I)%INEIGHB(K), X1, Y1, NUMBER + 1, IRECEXS(I)%LOCALREF(K), NUM_DG_RECONSTRUCT_DOFS - 1)
            ELSE
                IEXSOLHIS(I)%SOL(K,1:NOF_VARIABLES)=U_C(IRECEXS(I)%LOCALREF(K))%VAL(1,1:NOF_VARIABLES)
            END IF
	END DO
    ...
    DO I=1,TNDL
        DO K=1,IEXCHANGES(I)%MUCHTHEYNEED(1)
            ...
            IF (DG == 1) THEN
                ...
                IEXBOUNDHIS(I)%U_CENTER_DG(K, 1:NOF_VARIABLES, 1:NUM_DG_DOFS)=U_C(IEXCHANGES(I)%LOCALREF(K))%VALDG(1,1:NOF_VARIABLES,1:NUM_DG_DOFS)
            ELSE
                IEXBOUNDHIS(I)%FACESOL_dg(K,1:NOF_VARIABLES)=ILOCAL_RECON3(IEXCHANGES(I)%LOCALREF(K))%ULEFT_dg(1:NOF_VARIABLES,IEXCHANGEs(I)%SIDEtheyNEED(K),IEXCHANGES(I)%QTHEYNEED(k)) !?
            END IF
