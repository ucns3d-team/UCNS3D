...
REAL,allocatable,DIMENSION(:)::RG,ARG
CHARACTER(LEN=20)::PROC,RESTFILE,PROC3
INTEGER:: prev_turbequation,INITIAL,III,i,k,jx,QQP,INC,kmaxe,jkn,ki,iterr,JX2,IDX,COUNT_1
...
!                     
                         DO INC=1,QQP
                            ...
                            U_C(I)%VALDG(1,1,:)=U_C(I)%VALDG(1,1,:)+MATMUL(INV_MASS_MATRIX(N,I,:,:),tempsol(1,:))
                         END DO 
                ...
                IF (DG.EQ.1)THEN
                        ...
                        DO INC=1,QQP
                           ...
                           basis_vector(1)=1.0d0
                             ...
                             tempsol(1,:)=LINEAR_INIT2D(N)*WEQUA3D(INC)*(ielem(n,i)%totvolume)*basis_vector(1:idegfree+1)
                         
                              U_C(I)%VALDG(1,1,:)=U_C(I)%VALDG(1,1,:)+MATMUL(INV_MASS_MATRIX(N,I,:,:),tempsol(1,:))
                             
!                             
!                             U_C(I)%VALDG(1,1,1)=U_C(I)%VALDG(1,1,1)+LINEAR_INIT2D(N)*WEQUA3D(INC)*VOLTEMP!QP_ARRAY(I,INC)%QP_WEIGHT  !THIS IS JUST THE INITIAL SOLUTION
!                             DO IDX=1,IDEGFREE
